...
export function handle_monster_action_event(io, socket, roomsPlaying, actionData) {
    ...
    skill.action_list.forEach((action) => {
        ...
        if (action.target_type === skillTargetType.OPPONENT) {
            actionResponse.push(monster_action_to_opponent_caculation(monsterPlayer, action.area_effect === skillArea.ALL ? monstersOpponent : monsterTarget, action));
        } else if (action.target_type === skillTargetType.ALLY) {
            actionResponse.push(monster_action_to_ally_caculation(monsterPlayer, monsterAlly, action));
        } else {
            actionResponse.push(monster_action_to_self_caculation(monsterPlayer, action));
        }
    });
