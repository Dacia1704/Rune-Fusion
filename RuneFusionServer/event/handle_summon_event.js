...
export async function handle_summon_event(io, socket, data) {
    try {
        ...
        for (let i = 0; i < summonData.summon_times; i++) {
            ...
            let amountGold = summonResult[summonResult.length - 1].gold;
            if (player.own_monster_list.includes(summonResult[summonResult.length - 1].monster_id)) {
                amountGold += 2000;
            } else {
                player.own_monster_list.push(summonResult[summonResult.length - 1].monster_id);
            }
