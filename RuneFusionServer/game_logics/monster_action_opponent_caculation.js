...
export default function monster_action_to_opponent_caculation(monsterPlayer, monsterOpponent, action, monster_base_data) {
    ...
    monsterOpponent.forEach((opponent) => {
        ...
        monster_affect.dam = Math.floor(monsterPlayer.data.stats.attack * action.percent_attack * buffAtk * 1.5 * (1 - effectiveDef / (effectiveDef + 400)));
        // cacul buff, debuff
        if (action.effect_skill.effect_type !== effectType.NONE && action.effect_skill.effect_type !== effectType.HEAL) {
            const ratio = Math.max(0.15, monsterPlayer.data.stats.accuracy - opponent.data.stats.resistance + 0.2);
            // const ratio = 1;
